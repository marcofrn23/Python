...
def get_next_target(page):
    # This function extracts links from a given page
...
def get_all_links(page):
    # This function returns an array with all the links extracted
    # from a given page
    links = []
    while True:
...
def add_to_index(index, keyword, url):
    # This procedure updates the web index adding/updating a bucket for a
    # given keyword adding the given url
    if keyword in index:
...
def add_page_to_index(index, url, content):
    # This procedures updates all the index looping the add_to_index procedure
    # for the given page
    words = content.split()
    for word in words:
...
def lookup(index, keyword):
    # This function returns all the urls associated with the given keyword
    if keyword in index:
...
def crawl_web(seed):
    # This function creates the web index and the web graph with all the found pages
    tocrawl = [seed] # Seed page
    crawled = []
    graph = {}
    index = {}
    while tocrawl:
        ...
        if page not in crawled:
            # Outlinks: All the links contained in a given page (exit-links)
            # Inlinks: All the links pointing to a given page (enter-links)
...
def compute_ranks(graph, numloops):
    d = 0.85 # damping factor, chosen by Google
    # This function corresponds to the PageRank algorithm
    # Starts a Stochastic process to determine with a given accuracy the ranks
    # of all the pages
    ranks = {}
    npages = len(graph)
    for page in graph:
        ranks[page] = 1.0 / npages # 'base case'
    # The rank measures the importancy of a page
    # This is equal to the probability a random web surfer gets to that page
    # The RWS can get to it jumping or following links:
    # The probability he jumps is set to 1-d, in that case all the other pages
    # have the same probability to be reached. If he instead follows a link,
    # the probability is governed by the pagerank of the page.
    # Reference to the absolute probability theorem
...
def lucky_search(index, ranks, keyword):
    # This is the function that allows a user to search with keywords:
    # The search engine searches inside the index, if the KW is found,
    # it returns the url with the highest pagerank
    best = ''
    bestrank = .0
    if keyword not in index:
        ...
        for url in urls:
            print url, ranks[url], best
            if url in ranks and ranks[url] > bestrank:
                best = url
                bestrank = ranks[url]
        return best
 #-------------------------------------------------------------------------#
...
# What to do
index, graph = crawl_web('<some page>')
